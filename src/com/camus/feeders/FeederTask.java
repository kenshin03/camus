...
public abstract class FeederTask {
	...
	if (linkedStory.getStoryDescription() == null) {
	    ...
	    if (matcher.find()) {
		storyDescription = matcher.group(1)
			.replaceAll("[\\s\\<>]+", " ").trim();
		if (storyDescription != null) {
		    linkedStory.setStoryDescription(storyDescription);
		    logger.debug("storyDescription: " + storyDescription);
	...
	try {
		...
		if (downloadImages) {
		    ...
		    logger.info("parseCrawledLinkedStory linkedStory.getStoryImageURL(): " + linkedStory.getStoryImageURL());
		    
		    if ((linkedStory.getStoryImageURL()!=null) && (storyURL != null)){
			String tempFileName = imageExtractor
			.downloadImageToTempFile(storyURL,
			...
			linkedStory.setStoryImageWidth(simpleImageInfo.getWidth());

			if (linkedStory.getStoryImageWidth() > linkedStory
				.getStoryImageHeight()) {
			    linkedStory.setStoryImageIsLandscape(true);
			} else {
			    linkedStory.setStoryImageIsLandscape(false);
			}
			*/
			linkedStory.setStoryImageURL(tempFileName);
			logger.info("parseCrawledLinkedStory setting tempFileName to setStoryImageURL: " + linkedStory.getStoryImageURL());
		    }
	...
	try {
		...
		if ((!contentTypeHeaderString
			...
			&& (!contentTypeHeaderString
				.equals("text/html; charset=UTF-8"))
			&& (!contentTypeHeaderString.equals("text/html"))
			&& (!contentTypeHeaderString.equals("application/xhtml+xml; charset=utf-8"))
		) {
